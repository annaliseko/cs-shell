 ...
 *
 * IF YOU ARE WORKING SOLO (not with a partner), your repository
 * itself identifies who you are; IGNORE THE FOLLOWING PARAGRAPH.
 *
 ...
 * in the pair above.
 * The SECOND ("PP1") person should hand in a tsh.c with the EXACT SAME
...
/* Reference Materials: 
   http://pastebin.com/DQyWJBWE
   Others throughout file
*/
...
void eval(char *cmdline)
{
  ...
  if (!builtin_cmd(argv)){
   ...
   sigprocmask(SIG_BLOCK,&mask,NULL);//blocking sigchld
   ...
   /*PARENT*/
     ...
     if (!is_bg) {
       addjob(jobs, pid, FG, cmdline); /*add child to the list in foreground*/
     }
     else { // for bg jobs
...
// stuff referenced: http://delorie.com/gnu/docs/glibc/libc_569.html

// WNOHANG, WUNTRACED, WIFSIGNALED, WIFEXITED and WIFSTOPPED
// all come from waitpid
void sigchld_handler(int sig)
{
  int stat; 
  pid_t pid;
